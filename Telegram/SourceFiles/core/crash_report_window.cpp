...
PreLaunchWindow::PreLaunchWindow(QString title) {
	...
	setWindowTitle(title.isEmpty() ? u"Monstergram"_q : title);
...
, _log(this)
, _close(this) {
	_label.setText(u"Monstergram Desktop başlatılamadı!\nAyrıntılı günlük aşağıdadır:"_q);
...
, _launch(std::move(launch)) {
	...
	if (_sendingState == SendingNoReport) {
		_label.setText(u"Monstergram Desktop son kullanımda düzgün kapatılmadı."_q);
	} else {
		_label.setText(u"Monstergram Desktop son kullanımda çöktü."_q);
	}
...
void LastCrashedWindow::saveReport() {
	QString to = QFileDialog::getSaveFileName(0, u"Monstergram Çökme Raporu"_q, QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation) + u"/report.telegramcrash"_q, u"Telegram crash report (*.telegramcrash)"_q);
	if (!to.isEmpty()) {
		QFile file(to);
...
void LastCrashedWindow::updateControls() {
	...
	QSize s(2 * padding + QFontMetrics(_label.font()).horizontalAdvance(u"Monstergram Desktop son kullanımda düzgün kapatılmadı."_q) + padding + _networkSettings.width(), h);
	if (s == size()) {
		resizeEvent(0);
	} else {
		resize(s);
